...
def white_noise():
    np.random.seed(0)
    return np.random.rand(512).astype(np.float64)
...
class TestFFS:
    ...
    def test_perfect_reconstruction(self, white_noise):
        score = _ext.metrics.ffs(white_noise, white_noise)
        assert abs(score - 1.0) < 1e-10

    @skip_if_no_ext
    def test_reconstruction_below_one_when_noisy(self, white_noise):
        noisy = white_noise + np.random.rand(512) * 0.1
        score = _ext.metrics.ffs(white_noise, noisy)
        assert score < 1.0

    @skip_if_no_ext
    def test_mismatched_lengths_raises(self, white_noise):
        with pytest.raises(Exception):
            _ext.metrics.ffs(white_noise, white_noise[:100])


class TestMSC:
    @skip_if_no_ext
    def test_identical_scales(self, white_noise):
        # Slice into 4 subscales
        scales = [white_noise[i*128:(i+1)*128] for i in range(4)]
        score = _ext.metrics.msc(scales, scales)
...
class TestFFRI:
    ...
    def test_random_returns_float(self, white_noise):
        labels = np.random.randint(0, 2, size=512).astype(np.int32)
        score = _ext.metrics.ffri(white_noise, labels)
    ...
    def test_constant_feature_zero_entropy(self, constant):
        ...
        score = _ext.metrics.ffri(constant, labels)
...
class TestSSRI:
    ...
    def test_constant_rfd_returns_one(self):
        rfd = np.ones(20, dtype=np.float64)
        score = _ext.metrics.ssri(rfd)
        assert score == pytest.approx(1.0)

    @skip_if_no_ext
    def test_high_var_rfd_below_one(self):
        rfd = np.array([1.0, 10.0, 0.5, 8.0, 2.0], dtype=np.float64)
        score = _ext.metrics.ssri(rfd)
        assert score < 1.0
...
class TestFRI:
    ...
    def test_all_above_threshold(self):
        ...
        assert _ext.metrics.fri(scores, 0.5) == pytest.approx(1.0)

    @skip_if_no_ext
    def test_none_above_threshold(self):
        scores = np.zeros(100, dtype=np.float64)
        assert _ext.metrics.fri(scores, 0.5) == pytest.approx(0.0)
...
class TestFractalDimensions:
    ...
    def test_higuchi_returns_positive(self, white_noise):
        d = _ext.dimensions.higuchi(white_noise, 10)
        assert d > 0.0
    ...
    def test_wavelet_dim_returns_positive(self, white_noise):
        d = _ext.dimensions.wavelet_dim(white_noise, 6)
        assert d > 0.0

    @skip_if_no_ext
    def test_lacunarity_returns_float(self, white_noise):
        lc = _ext.dimensions.lacunarity(white_noise, 8)
        assert isinstance(lc, float)

    @skip_if_no_ext
    def test_lyapunov_kaplan_yorke(self):
        exps = np.array([0.5, 0.2, -0.1, -1.0], dtype=np.float64)
        d = _ext.dimensions.lyapunov_dim(exps)
        assert d > 0.0

    @skip_if_no_ext
    def test_multifractal_spectrum_has_keys(self, white_noise):
        result = _ext.dimensions.multifractal_spectrum(white_noise)
        assert "alpha" in result and "f" in result

    @skip_if_no_ext
    def test_correlation_dim_positive(self, white_noise):
        d = _ext.dimensions.correlation_dim(white_noise, 2, 1)
...
class TestFDNN:
    ...
    def test_forward_output_shape(self):
        ...
        cfg.input_dim = 32
        cfg.output_dim = 8
        state = _ext.neural.fdnn_init(cfg, 0)
        x = np.random.rand(32).astype(np.float64)
        out = _ext.neural.fdnn_forward(x, state, cfg)
        assert len(out) == 8

    @skip_if_no_ext
    def test_fractal_loss_nonnegative(self):
        cfg = _ext.neural.FDNNConfig()
        state = _ext.neural.fdnn_init(cfg, 1)
        x = np.random.rand(cfg.input_dim).astype(np.float64)
        loss = _ext.neural.fdnn_fractal_loss(x, state, cfg)
        assert loss >= 0.0
...
class TestMSFCN:
    ...
    def test_forward_returns_vector(self):
        ...
        cfg.kernel_size = 4
        cfg.num_scales = 3
        state = _ext.neural.msfcn_init(cfg, 0)
        sig = np.random.rand(128).astype(np.float64)
        out = _ext.neural.msfcn_forward(sig, state, cfg)
        assert len(out) > 0

    @skip_if_no_ext
    def test_scale_consistency_loss_nonneg(self):
        cfg = _ext.neural.MSFCNConfig()
        state = _ext.neural.msfcn_init(cfg, 0)
        sig = np.random.rand(128).astype(np.float64)
        loss = _ext.neural.msfcn_scale_consistency_loss(sig, state, cfg)
        assert loss >= 0.0
...
class TestSNNFT:
    ...
    def test_step_returns_spikes(self):
        ...
        state = _ext.neural.snnft_init(cfg, 0)
        spikes = np.zeros(8, dtype=np.float64)
        spikes[0] = 1.0
        out = _ext.neural.snnft_step(spikes, state, cfg)
        assert len(out) == 1  # one soma
